...
class RedisController:
    ...
    def list_celery_tasks(self) -> tuple[dict[str, Any], int]:
        ...
        try:
            ...
            for key in r.scan_iter(match=pattern):
                ...
                tasks.append({"key": key_str, "task_id": task_id, "meta": meta_str})
    ...
    def list_redis_keys(self) -> tuple[dict[str, Any], int]:
        ...
        try:
            ...
            for key_bytes in r.scan_iter(match=pattern):
                ...
                tasks.append({"key": key_str, "task_id": task_id, "created_at": created_at})
    ...
    def delete_redis_key(self, task_id: str) -> tuple[dict[str, Any], int]:
        ...
        try:
            ...
            if deleted:
                return {"task_id": celery_task_id, "status": "SUCCESS"}, 200
            else:
                return {"task_id": celery_task_id, "status": "NOT FOUND"}, 404

        except Exception as exc:
            logger.error(
                "Error deleting Redis key",
                task_id=task_id,
                error=str(exc),
                error_type=type(exc).__name__,
                stacktrace=traceback.format_exc(),
            )
            return {"task_id": task_id, "status": "ERROR"}, 422
