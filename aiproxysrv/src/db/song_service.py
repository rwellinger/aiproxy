...
class SongService:
    ...
    def update_song_status(
        self, task_id: str, status: str, progress_info: dict[str, Any] | None = None, job_id: str | None = None
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    ...
                    status=status,
                    job_id=job_id,
                    has_progress_info=bool(progress_info),
                )
                return True
    ...
    def bulk_cleanup_completed_songs(self, limit: int = 100) -> dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                completed_songs = db.query(Song).filter(Song.status == SongStatus.SUCCESS.value).limit(limit).all()

                cleanup_results = {"cleaned": 0, "errors": 0, "not_found": 0, "task_ids": []}
                ...
                logger.info(
                    "bulk_cleanup_completed",
                    cleaned=cleanup_results["cleaned"],
            ...
            logger.error(
                "bulk_cleanup_failed", error=str(e), error_type=type(e).__name__, stacktrace=traceback.format_exc()
            )
            return {"error": str(e)}
